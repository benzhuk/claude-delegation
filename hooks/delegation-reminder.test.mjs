...
// ─────────────────────────────────────────────────────────────────────────────
// MAJOR 3 — every subagent gets the card; only the report-writing roles get the line
// ─────────────────────────────────────────────────────────────────────────────

test('MAJOR 3: every subagent type gets the card, and only builder/reviewer/integrator get the report line', () => {
  const home = fixtureHome();
  const root = project();

  for (const agent_type of ['delegation:builder', 'delegation:reviewer', 'delegation:integrator', 'builder', 'Reviewer']) {
    const r = runHook('SubagentStart', home, { cwd: root, input: { agent_id: 'agent-1', agent_type } });
    assert.equal(r.status, 0, agent_type);
    assert.equal(r.json.hookSpecificOutput.hookEventName, 'SubagentStart', agent_type);
  ...
  for (const agent_type of ['Explore', 'Plan', 'general-purpose', 'mrc-memo-writer', 'other-plugin:writer', undefined]) {
    const r = runHook('SubagentStart', home, { cwd: root, input: { agent_id: 'agent-2', agent_type } });
...
test('MAJOR 4: a malformed card produces exactly one systemMessage at SessionStart and nothing elsewhere', () => {
  ...
  for (const [event, input] of [['SubagentStart', { agent_type: 'delegation:builder' }], ['PostToolBatch', {}]]) {
    const r = runHook(event, home, { cwd: root, input });
    assert.equal(r.stdout, '', `${event} must not repeat the notice`);
  }
});
...
test('MAJOR 1: with a switch file present the hook writes, deletes and reads nothing', () => {
  ...
  for (const name of [MASTER_SWITCH, `ws-off-${SWITCH_NAME}`]) {
    ...
    for (const [event, input] of [
      ...
      ['SubagentStart', { agent_type: 'delegation:builder' }],
      ['PostToolBatch', {}],
...
test('the card payload is under budget wherever it is injected', () => {
  const home = fixtureHome();
  const root = project();
  for (const [event, input] of [['SessionStart', { source: 'startup' }], ['SubagentStart', { agent_type: 'delegation:builder' }]]) {

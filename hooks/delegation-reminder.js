...
// `compact` source, which is how a session re-enters after compaction) and once per 40 tool batches
// during a long autonomous stretch.
//
// A SUBAGENT DOES NOT GET THE CARD AT SPAWN (build 0921-S ruling). The frozen tip this file was
// salvaged from also wired `SubagentStart`; that ruling was reopened and reversed for this build: a
// subagent gets its goal from its mandate, which the orchestrator writes, not from a hook injecting
// the project card into a context it does not share history with. A long-running subagent still
// reaches the card through its own PostToolBatch counter below (its `agent_id` keys its own tally),
// which is the one mechanism this build keeps.
//
...
async function handle(event, input) {
  ...
  // No SubagentStart handler (build 0921-S ruling, reversing the frozen tip this file was salvaged
  // from): a subagent's context starts from its mandate, not from this hook. A subagent that runs
  // long enough still reaches the card through its own PostToolBatch tally below.

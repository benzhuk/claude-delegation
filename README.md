...
- **`/delegation:bearings`** — assess a goal from bounded evidence, choose `CONTINUE`,
  ...
  SessionStart metadata is available and positively classifies a Codex session as a
  lead, it receives the same card and due/unknown advisory at SessionStart and every
  eligible UserPromptSubmit; prompts without that classification or a usable card receive
  no new advisory. Confirmed children receive
...
- 0.20.9 — Codex lead source parity: when native SessionStart metadata is available and
  ...
  per-prompt tradeoff uses roughly 1,200 bytes of card context plus a bounded advisory,

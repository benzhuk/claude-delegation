...
## Changelog
- 0.20.8 — GOALS.md status updates, no code change: records the four-host 0.20.6 install of
  2026-09-24 (Windows, Mac, Hetzner, Netcup) with the 0.20.7 card-cap change installed nowhere
  yet as of this writing; records the 8:25 AM scheduled pickup on 2026-09-24 that ran unattended
  and returned PICKUP_NO_ACTION with Done false, with a checked-Done handback still never having
  happened; and records that the 2026-09-24 bearings RE-PLAN was run by the lead and so does not
  count as independent, with the STOP line's RE-PLAN count starting over from the next
  independent run.

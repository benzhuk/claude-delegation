...
//   checked out (the fix revision). A load/import error, zero real tests, or a crash at
//   either revision is inconclusive: exit 2, never 0. A base run that already passes (the
//   fix made no difference the test can see) is a conclusive non-reproduction: exit 1. A
//   base run that reproduces but whose fix-revision run does not pass is also exit 1
//   (conclusively: not a proven fix), unless the fix-revision run is itself inconclusive,
//   which is exit 2 (assumption; the spec's three named fixture cases only pin the base
//   run's three outcomes — see the T3 report).
// - Last step: `git worktree remove --force <tmp>` on the exact path this run created,
//   verified before removal. No network, no writes outside the temp worktree, never
//   `stash`, `reset`, `clean`.
...
// Parses `node --test --test-reporter=tap` output. `testFileBasename` lets us recognize
// the one shape plain summary counts can't: a file with zero real `test()` calls is
// reported by node itself as a single passing top-level "test" named after the FILE (not
// a real test name) — measured directly against node v24.18.0 (see the T3 report for the
// three probe transcripts this shape and the ERR_ASSERTION/ERR_TEST_FAILURE split below
// were taken from).
export function parseTapCounts(output, testFileBasename) {
  const testsMatch = /^# tests (\d+)/m.exec(output);
  const passMatch = /^# pass (\d+)/m.exec(output);
  const failMatch = /^# fail (\d+)/m.exec(output);
  const hasAssertionFailure = /code:\s*'ERR_ASSERTION'/.test(output);
  const topLevelLines = output
    .split(/\r?\n/)
    .filter((l) => /^(not )?ok \d+ - /.test(l));
  const zeroRealTests =
    testFileBasename !== undefined &&
    topLevelLines.length === 1 &&
    topLevelLines[0].replace(/^(not )?ok \d+ - /, "").trim() === testFileBasename;
  return {
...
// Stripped unconditionally, not just when detected, so this holds regardless of how deep
// prefix-test.mjs itself is invoked from.
export function classifyRun(cwd, testFile) {
  ...
  const counts = parseTapCounts(output, path.basename(testFile));

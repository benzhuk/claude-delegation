...
P2 review adjudication: per-response records establish observed usage, not completeness of all responses in a resumed/mixed-schema session. Do not invent correlation rules or reject ordinary coexisting token_count events. Until complete coverage is verified, Codex report line one is UNSUPPORTED for a complete census, leadTokens states unsupported coverage, and separately named observedLeadTokens may preserve the verified numeric observation. JSON exposes coverageSupported: false and its reason. Work-record must refuse this report as a complete census; this build uses --no-census with the precise coverage/turn/child limitations and attaches diagnostic observations separately. No token records in the marker window means unknown, not a measured zero, including native counts and rate fields. Conflicting repeated response ids fail visibly; identical repeats may deduplicate. Metadata preludes do not establish build-marker boundaries. Accept all other mechanical P2 review corrections, including nulls for unsupported JSON values and explicit no child-input support.

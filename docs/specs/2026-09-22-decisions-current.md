...
- `scripts/goals-mirror.mjs publish --repo <path> --parent <page-id>` requires `--current <read-of-goals-page.md>` (a read taken seconds earlier; `--current none` only when no Goals child page exists yet). It runs the reader over that read and refuses (exit 1, printing the note lines) if there is any owner note (any `COMMENTED` line, or any `UNATTACHED` line whose kind is comment). Only then does it render and call `node ~/.claude/scripts/notion.js publish <parent> "Goals" <tmpfile>`. That call is a whole-page replace, which is why the refusal comes first. It also refuses (exit 1) on the source-file condition in F6.
...
- Builders branch from `main` (889887a or later), not `integrate/0921`. T3 adds a `## Releasing` section at the end of `docs/pane-setup.md`: bump, commit, push, then `node skills/decisions/scripts/goals-mirror.mjs publish --repo . --parent <goals_parent_page> --current <read>`, and paste its output line into the release report. No runner brief template exists; none is created. Status lines in GOALS.md change only in a release commit; this step is what keeps the page current.

### M3. Hand-back check

- `scripts/decisions-handback.mjs --decisions <read.md> --goals <read.md> --repo <path>` runs the reader over both pages; it works out head the same way as M2 (`--repo <path>`), `--head <x>` is a test override only, and two shas match when one is a prefix of the other. It exits 0 only when, across both pages: no COMMENTED, TICKED, AMBIGUOUS or DUE line; no UNATTACHED line; no WARN line. On the decisions page, `DONE` must be `false` when `DECISIONS` > 0 and `true` when `DECISIONS` is 0; `absent` is blocked. The goals page is exempt from the Done rule. The goals mirror sha must match (the F6 rule). Otherwise prints the offending lines and exits 1. BLIND exits 3. With `~/.agents/ws-off-decisions` or `ws-off` present, a blocked result exits 0 and still prints `HANDBACK blocked`; BLIND still exits 3.
- The skill rule: the decisions URL is given to the owner only after this exits 0 on a read taken seconds earlier; the lead runs it through a Sonnet runner (a read is two `notion.js read` calls). On exit 0, `decisions-handback.mjs` prints, as its line just before `HANDBACK ok`: `Decisions waiting: <DECISIONS>, notes logged today: <count of decisions-page lines matching /^\s*- Your note, <today as M-D, America/New_York>:/>, goals mirror at <page sha>`. `--today <M-D>` is the test override. The lead pastes that line verbatim as the last line of the hand-back message, so the line cannot exist unless the check ran.

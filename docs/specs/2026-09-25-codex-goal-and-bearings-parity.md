...
1. For a confirmed lead (not a child), on SessionStart and UserPromptSubmit, append to `additionalContext` the shared Claude content (not its event schedule): the rendered goal card from `scripts/goal-card.mjs` for the checkout in `cwd`, and the bearings notice from `bearings-state.mjs` `check()` when due or unknown. Same kill switches as the Claude side (`ws-off`, `ws-off-goalcard`, `ws-off-bearings` under `$AGENTS_HOME`, fail open, never block).
...
Lead adjudication: docs/specs/codex-parity-1/contracts.md resolves the independent spec review, including output preservation, startup evidence limits, explicit per-prompt cost and unsupported census handling. It controls these implementation details.

...
Claude Code can show a bounded due/unknown advisory on SessionStart and its existing PostToolBatch route. A positively classified Codex lead can receive the same shared due/unknown advisory with its goal card on SessionStart and UserPromptSubmit; SessionStart can also show the one-line advisory in the pane. Confirmed Codex children receive none of these new goal or bearings effects. Unknown native identity keeps only its existing inbox/continuation behavior, so child-work detection remains incomplete. Codex has no goal-card or bearings cadence on PostToolUse, Stop, or Interrupt; Claude retains its own existing cadence. Neither host runs an assessment while idle, schedules work, or treats a notice as completion.
